<script>
    $(document).ready(function() {
        $('#subcomments_<?php echo $comment['id'];?>').load("//"+"<?php echo MAIN_URL; ?>"+"/viewwallpost/getsubcomment.inc.php/?id=<?php echo $comment['id'];?>&selauthid=<?php echo $selectedAuthor;?>");
        ...
        $('#add_subcomm_<?php echo $comment['id']; ?>').click(function(e) {  
            $.ajax({
                ...
                success: function(response) { //Данные отправлены успешно
                    ...
                    $('#result_form_<?php echo $comment['id']; ?>').prepend('<div class="all-sub-comments"><div class="comment m-content "><div class="comment-person-pl">'+avatar+'<div class="comment-person-name"><a href="../account/?id='+result.idauthor+'">'+result.authorname+'</a><br><span class="comment-date">Только что</span></div></div><div class="comment-text"><p><form action = "?" method = "post"><div><input type = "hidden" name = "id" value = "'+result.id+'"><input type = "hidden" name = "idarticle" value = "'+result.idarticle+'"><input type = "submit" name = "action" class="btn_2" value = "Редактировать"><input type = "submit" name = "action" class="btn_1" value = "Del"></div></form></p>'+result.text+'</div></div><div class = "comment-line"></div></div>');
